...
for tr=1:N
    ...
    for n=1:ndim
        ...
        regterm = (hmm.HRF(tr).alpha.shape(n) /  hmm.HRF(tr).alpha.rate(n)) * hmm.HRF(tr).prior.B.iS;  %%% iS ???
        precision = hmm.HRF(tr).sigma.shape(n) / hmm.HRF(tr).sigma.rate(n);
        hmm.HRF(tr).B.S(:,:,n) = inv(precision*(GG + V) + regterm);
        hmm.HRF(tr).B.mu(:,n) = hmm.HRF(tr).B.S(:,:,n) * ...
            (precision * G' * y(:,n) + regterm * hmm.HRF(tr).prior.B.mu);
        ...
        hmm.HRF(tr).sigma.rate(n) = hmm.HRF(tr).prior.sigma.rate(n) + ...
            ...
            0.5 * sum(sum((V .* (hmm.HRF(tr).B.mu(:,n) * hmm.HRF(tr).B.mu(:,n)') ))) + ... 
